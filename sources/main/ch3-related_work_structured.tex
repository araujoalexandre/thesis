...
% \newpage
%
% \todo{contradictoire: improved accuracy and higher overfitting}
% \todo{citer alexnet et attention is all your need}
...
As seen in the Introduction (\Cref{chapter:ch1-introduction}), scaling up networks can lead to better accuracy~\cite{krizhevsky2012imagenet,he2016residual,vaswani2017attention}.
...
\todotext{developper plus les Tensor Train}
\todotext{add some words on the shortcomings of these methods}
...
In the context of reducing the parametrization of neural networks, \citet{cheng2015exploration} proposed to replace the weight matrix of a fully connected layer by the product of a circulant and a diagonal matrix leading to following structured layer:
\begin{equation}
...
% \todo{du coup, ça n'est plus une circulante}
% \todo{lequels theoretical guarantees}

%
% Due to this decomposition, the circulant matrix to be represented by only $n$ values instead of $n^2$.
% Furthermore, 
%
% Using circulant matrices offer several advantages  
%
% This layer offer several advantages, first, because 
%
% it can be  due to the properties of the circulant matrices.
%
% Because the circulant matrix can be represented by only $n$ values, this layer offer a reduction of parameters fr
%
% leading the layer to be parameterized by only $n$ values instead of $n^2$ and the matrix-vector product to be efficiently computed with the Fast Fourier Transform (see \Cref{algorithm:ch2-matrix_vector_product_circulant_matrix}).
%
%
% \todo{et alors ?}
...
\begin{theorem}[Krylov Decomposition~\citet{pan2003inversion,sindhwani2015structured}] \label{theorem:ch3-krylov_decomposition}
  ...
  \removespace
\end{theorem}
\todotext{donner example Toeplitz-like}
\todotext{def reflection matrix}
...
More precisely, they proposed to parameterize neural networks layers as in \Cref{equation:ch3-toeplitz_like_matrix_decomposition} and learn the displacement factors $\Gmat$ and $\Hmat$ by gradient descent. 
