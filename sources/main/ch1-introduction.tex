...
The main reasons for this success are threefold.
First of all, they used a model with more than 60 million parameters which was significant for the computational resources of the time.
...
As we can see, the accuracy of the models have dramatically improved, generally at the cost of their size.
...
This discovery lead them to develop a new scaling method that balance all dimensions of network width, depth, and resolution, leading to an efficient neural network with similar or higher accuracy than existing ones.  
...
The latest GPT-3 model, proposed by~\citet{brown2020language}, culminates at 175 billion parameters and requires 355 years of training on a single GPU and \$4,600,000 to train on a cloud-based compute platform.
...
Neural networks, when implemented in a critical decision process, need to be compact, cost effective, secure.
...
In this thesis, we focus on the design and the training of \emph{neural networks}.
...
More precisely, a neural network is a function $f_{\Theta} : \Rbb^n \rightarrow \Rbb^m$ parameterized by $\Theta$ which can be expressed as follows:
\begin{equation}
  f_{\Theta}(\xvec) = \psi^{(\depth)} \circ \rho \circ \psi^{(\depth-1)} \cdots \circ \psi^{(2)} \circ \rho \circ \psi^{(1)} (\xvec)
  \label{equation:ch1-neural_network}
\end{equation}
where $\depth$ corresponds to the \emph{depth} of the network (\ie, the number of layers), $\psi^{(i)}$ are linear functions and $\rho$ is a non-linear function (\Cref{definition:ch2-neural_networks} present a more formal definition a neural networks).
...
As we saw in the previous section, large neural networks have achieve state-of-the-art performances in a variety of domains such as computer vision and natural language processing.
...
% TODO: make difference of training and inference ? 
...
Thus, building compact and cost effective neural network have been an important goal in order to reduce training time, reduce cost and allow for faster research and development.
...
The dimension of the input is usually large, for example, with the ImageNet dataset $n = 224^2 \times 3$ and $m = 1000$ leading to a two-layer fully connected neural network with more than 150 million parameters\footnote{The resolution of $224^2 \times 3$ with the ImageNet dataset is usually used but sometimes higher resolution can be used \cite{tan2019efficientnet}.}.
...
An example of neural networks with specific linear operations are \emph{Convolutional Neural Networks} (CNN)~\cite{lecun1998gradient,krizhevsky2012imagenet,he2016deep,tan2019efficientnet} which achieve state-of-the-art result for computer vision tasks.
They use convolutional layers which are specific to image processing and use very few parameters.  
Whereas a classical linear layer with a dense matrix will have $n \times n$ parameters, a convolution layer only have $k \times k$ parameters where $k << n$ is the kernel size and is usually small (\eg 3 or 5 for classical convolutional layers).
A convolutional neural networks is a type of \emph{structured} neural network due to the structure of the convolution operation.
Indeed, the convolution operation can be represented with with a structured matrix \ie, matrix that can be represented with less than $n^2$ parameters.
...
The \Cref{figure:ch1-adversarial_image_example} gives an examples of an adversarial attacks on an image.
