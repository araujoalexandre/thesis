%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Introduction on Supervised Learning}
...
These features are used as input to the function and the function outputs a value characterizing the taste of the fruit. 
...
\end{equation}
where $L: \Yset \times \Yset \rightarrow \Rbb_{+}$ is a \emph{loss function} which measure the correctness of the hypothesis.
For example, for classification problems, we can define $L$ as: %$L(h(\xvec), y) = \mathds{1}\big[ h(\xvec) \neq y \big]$.
\begin{equation}
  L(h(\xvec), y) = \mathds{1}\big[ h(\xvec) \neq y \big]
\end{equation}
...
\begin{equation}
  R_{\mathcal{S}}(h) \triangleq \frac{1}{|\mathcal{S}|} \sum_{(\xvec, y) \sim \mathcal{S}} L\big( h(\xvec), y \big) \enspace.
\end{equation}
...
In the next section, we will present a classic regularization function for neural networks and we will introduce a new regularization scheme in \Cref{chapter:ch5-lipschitz_bound}.
...
They used the MNIST dataset \cite{lecun1998gradient} consists of black and white images of size $28 \times 28$ pixels (\Cref{figure:ch2-mnist-database} presents a sample of images from the MNIST database).
Their goal was to develop an algorithm that takes a vector as input and produces the digit from 0 to 9 as the output.
Although simple for a human, this task is a non-trivial problem for computers due to the uniqueness of each image. 
To solve this problem, \citet{lecun1998gradient} trained a neural network based on the images of the MNIST dataset and their label.
...
\begin{definition}[Neural Network]
  ...
  Let $\Xset \subset \Rbb^{\dimw^{(1)}}$ and $\Yset \subset \Rbb^{\dimw^{(\depth)}}$ be the input space and output space respectively. 
  ...
  \label{definition:ch2-neural_networks}
\end{definition}
...
\noindent
The \Cref{figure:ch2-activation_functions} present the graphical representation of the activation functions presented above.
