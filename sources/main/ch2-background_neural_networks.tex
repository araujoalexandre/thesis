...
The ERM paradigm assumes that an hypothesis $h_\mathcal{S}$ that minimizes the risk $R_\mathcal{S}$ will also minimize the true risk $R_\mathcal{D}$.
...
\end{equation}
where the function $m_\mathcal{H}$, called \emph{the sample complexity}, measures the minimal number of examples needed to ensure that with probability of at least $1 - \delta$ the sample verify \Cref{equation:ch2-eps_representative}. 
...
\end{theorem}
\noindent
Note that we can also write the bound from~\Cref{theorem:ch2-sample_complexity_bound} as follows:
\begin{equation}
...
A fundamental question of the ERM paradigm remains: \emph{how to choose the correct hypothesis class for which $\text{ERM}_\mathcal{H}$ will not lead to overfitting?} 
