...
For example, for classification problems, we can use the 0-1 loss define as: %$L(h(\xvec), y) = \mathds{1}_\big[ h(\xvec) \neq y \big]$.
\begin{equation}
...
The ERM paradigm assumes that an hypothesis $h_\Sset$ that minimizes the risk $R_\Sset$ will also minimize the true risk $R_\Dset$.
...
\end{equation}
where the function $m_\Hset$, called \emph{the sample complexity}, measures the minimal number of examples needed to ensure that with probability of at least $1 - \delta$ the sample verify \Cref{equation:ch2-eps_representative}.
...
Now we can state a result which gives the sample complexity measure the maximum value for which the hypothesis class has \emph{uniform convergence property}.
...
\end{theorem}
\noindent
Note that we can also write the bound from~\Cref{theorem:ch2-sample_complexity_bound} as follows, $\forall h \in \Hset$, with probability $1 - \delta$, we have:
\begin{equation}
...
\end{itemize}
Recall that the main goal is to minimize the true risk $R_\Dset (h_\Sset)$, however, \Cref{equation:ch2-bias_complexity_tradeoff} shows a tradeoff called the \emph{bias-complexity tradeoff}.
...
The \emph{adversarial empirical margin risk} is defined similarly as in~\Cref{equation:ch2-emprical_margin_risk}.
...
\begin{theorem}[\citet{farnia2018generalizable}]
  ...
  Let $N_\Omega: \Rbb^{w^{(1)}} \rightarrow \Rbb^{w^{(\depth)}}$ be a neural network parameterized by $\Omega$ of depth $\depth$ and width $w$ following the~\Cref{definition:ch2-neural_networks} where $\left( \Wmat^{(1)}, \dots, \Wmat^{(\depth)} \right)$ are the weights matrices.
