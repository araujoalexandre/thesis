...
To recover the block Toeplitz matrix from its generating function, we use the Toeplitz operator defined in \Cref{equation:ch2-toeplitz_operator}; therefore, we have $\Tmat(F_\Bmat) = \Bmat$.
...
\begin{theorem}[Bound on the singular values of Toeplitz matrices]
  ...
  \end{align}
  \label{theorem:toeplitz_sup_singular}
\end{theorem}

\Cref{theorem:toeplitz_sup_singular} is a direct application of Lemma 4.1 by~\citet{gray2006toeplitz} for real Toeplitz matrices. 
\begin{theorem}[Bound on the singular values of block Toeplitz matrices ~\citet{gutierrez2012block}]
  ...
  \end{align}
\label{theorem:block_toeplitz_sup_singular}
\end{theorem}
