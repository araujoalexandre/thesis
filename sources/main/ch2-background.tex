...
\begin{definition}[Neural Network]
  ...
  \end{equation}
  % where $d$ corresponds to the depth of the network (\ie, the number of layers), $\Omega$ is the set of weights matrices and bias vectors $\Omega = \left\{ \left( \Wmat^{(1)}, \bvec^{(1)} \right) \dots \left( \Wmat^{(d)}, \bvec^{(d)} \right) \right\}$.
...
\begin{theorem}[\citet{gray2006toeplitz}]
...
% An $n\times n$ Toeplitz matrix $\mathbf A$ is fully determined by a two-sided sequence of scalars: $\{a_h\}_{h \in P}$, whereas an $nm\times nm$ block Toeplitz matrix $\Bmat$ is fully determined by a two-sided sequence of blocks $\{\Bmat_h\}_{h \in P}$ and where each block $\Bmat_h$ is an $m \times m$ matrix.  
%
% \begin{equation}
