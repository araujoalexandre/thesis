...
Let $\{a_h\}_{h \in \Pset}$ be the sequence of coefficients of the Toeplitz matrix $\Amat \in \Rbb^{n\times n}$ and let $\{\Bmat^{(h)}\}_{h \in \Pset}$ be the sequence of $m\times m$ blocks of the block Toeplitz matrix $\Bmat$.
The matrices are therefore denoted $\Amat = \leftmat a_{j-i} \rightmat_{i,j \in \Pset}$ and $\Bmat = \leftmat \Bmat^{(j-i)} \rightmat_{i,j \in \Pset}$ where $\Pset = \left\{ -n+1, \cdots, n-1 \right\}$.
We can then defined their generating functions $f(\omega) = \sum_{h \in \Pset} a_h e^{\ci h \omega}$ and the matrix-valued function $F(\omega) = \sum_{h \in \Pset} \Bmat^{(h)} e^{\ci h \omega}$ which are the \emph{inverse Fourier transforms} of the sequences $\{a_h\}_{h \in \Pset}$ and $\{\Bmat^{(h)}\}_{h \in \Pset}$, with $\omega \in \Rbb$.
...
\begin{equation} \label{equation:expression_toeplitz_matrix}
  \Tmat(g)  \triangleq\leftmat\frac{1}{2\pi}\int_{0}^{2\pi}e^{-\ci(i-j)\omega}g(\omega)\,d\omega\rightmat_{i,j \in \Pset_0} \enspace.
\end{equation}
...
% The complex-valued function $f(\omega) = \sum_{h \in \Pset} a_h e^{\ci h \omega}$ and the matrix-valued function $F(\omega) = \sum_{h \in \Pset} \Bmat_h e^{\ci h \omega}$ are the \emph{inverse Fourier transforms} of the sequences $\{a_h\}_{h \in \Pset}$ and $\{\Bmat_h\}_{h \in \Pset}$, with $\omega \in \Rbb$.
...
% An $n \times n$ Toeplitz matrix $\Amat$ is fully determined by a two-sided sequence of scalars: $\{a_h\}_{h \in \Pset}$, whereas an $nm\times nm$ block Toeplitz matrix $\Bmat$ is fully determined by a two-sided sequence of blocks $\{\Bmat^{(h)}\}_{h \in \Pset}$, where $\Pset = \{-n+1,\dots, n-1\}$ and where each block $\Bmat_h$ is an $m \times m$ matrix.
% \begin{equation*}
%   \Amat = \leftmatrix
...
% In the remainder, we will use the standard notation $\leftmat\ \cdot\ \rightmat_{i,j \in \Pset}$ to construct (block) matrices.
% For example, $\Amat = \leftmat a_{j-i} \rightmat_{i,j \in \Pset}$ and $\Bmat = \leftmat \Bmat^{(j-i)} \rightmat_{i,j \in \Pset}$.
...
% Let $\{a_h\}_{h \in \Pset}$ be the sequence of coefficients of the Toeplitz matrix $\Amat \in \Rbb^{n\times n}$ and let $\{\Bmat_h\}_{h \in \Pset}$ be the sequence of $m\times m$ blocks of the block Toeplitz matrix $\Bmat$.
% The complex-valued function $f(\omega) = \sum_{h \in \Pset} a_h e^{\ci h \omega}$ and the matrix-valued function $F(\omega) = \sum_{h \in \Pset} \Bmat_h e^{\ci h \omega}$ are the \emph{inverse Fourier transforms} of the sequences $\{a_h\}_{h \in \Pset}$ and $\{\Bmat_h\}_{h \in \Pset}$, with $\omega \in \Rbb$.
% From these two functions, one can recover these two sequences using the standard Fourier transform:
% \begin{equation}
%     a_h = \frac{1}{2\pi} \int_0^{2\pi} e^{-\ci h \omega} f(\omega) d\omega \quad \quad \Bmat_h = \frac{1}{2\pi} \int_0^{2\pi} e^{-\ci h \omega} F(\omega) d\omega .
...
% \end{equation}
%
% Note that if $f$ is the inverse Fourier transform of $\{a_h\}_{h \in \Pset}$, then $\Tmat(f)$ is equal to $\Amat$.
...
Let $\Dmat(f) = \leftmat \Dmat_{i,j}(f)\rightmat_{i,j \in \Pset_0}$ where $\Dmat_{i,j}$ is defined as:
\begin{equation} \label{equation:doubly_block_toeplitz_operator}
  \Dmat_{i,j}(f) = \leftmat \frac{1}{4\pi^{2}} \int_{[0,2\pi]^{2}} e^{- \ci \left((i-j)\omega_{1}+(k-l)\omega_{2}\right)}f(\omega_{1},\omega_{2})\,d(\omega_{1},\omega_{2}) \rightmat_{k,l \in \Pset_0'} \enspace.
\end{equation}
where $\Pset_0 = \{0, \dots, n-1 \}$ and $\Pset_0' = \{0, \dots, m-1 \}$.
...
\begin{maintheorem}[Bound on the largest singular value of a Doubly-Block Toeplitz Matrix] \label{theorem:doubly_block_teoplitz_sup_singular}
  ...
  \begin{equation}\label{equation:muli_variate_poly_on_M}
      f(\omega_1, \omega_2) \triangleq \sum_{h_1 \in \Pset} \sum_{h_2 \in \Pset'} d_{h_1, h_2} e^{\ci (h_1 \omega_1 + h_2 \omega_2)},
  \end{equation}
  where $d_{h_{1},h_{2}}$ is the ${h_2}^\textrm{th}$ scalar of the ${h_1}^\textrm{th}$ block of the doubly-Toeplitz matrix $\Dmat(f)$, and where $\Pset' = \{-m+1, \dots, m-1\}$.
\end{maintheorem}
...
Let $\Hmat^{\alpha_p} (f) = \leftmat \Hmat^{\alpha_p}_{i,j}(f) \rightmat_{i,j \in \Pset_0}$ where $\Hmat^{\alpha_p}_{i,j}$ is defined as:
\begin{equation}
  ...
  \rightmat_{k,l \in \Pset_0} \enspace.
\end{equation}
...
\todotext{fix captions: \Cref{figure:ch5-jacobian_distribution_v1} and \Cref{figure:ch5-jacobian_distribution_v2}}
...
\todotext{fix captions: \Cref{figure:ch5-attacks_pgd} and \Cref{figure:ch5-attacks_cw}}
