...
% The complex-valued function $f(\omega) = \sum_{h \in \Iset} a_h e^{\ci h \omega}$ and the matrix-valued function $F(\omega) = \sum_{h \in \Iset} \Bmat_h e^{\ci h \omega}$ are the \emph{inverse Fourier transforms} of the sequences $\{a_h\}_{h \in \Iset}$ and $\{\Bmat_h\}_{h \in \Iset}$, with $\omega \in \Rbb$.
...
% An $n \times n$ Toeplitz matrix $\Amat$ is fully determined by a two-sided sequence of scalars: $\{a_h\}_{h \in \Iset}$, whereas an $nm\times nm$ block Toeplitz matrix $\Bmat$ is fully determined by a two-sided sequence of blocks $\{\Bmat^{(h)}\}_{h \in \Iset}$, where $\Iset = \{-n+1,\dots, n-1\}$ and where each block $\Bmat_h$ is an $m \times m$ matrix.
% \begin{equation*}
%   \Amat = \leftmatrix
...
% In the remainder, we will use the standard notation $\leftmat\ \cdot\ \rightmat_{i,j \in \Iset}$ to construct (block) matrices.
% For example, $\Amat = \leftmat a_{j-i} \rightmat_{i,j \in \Iset}$ and $\Bmat = \leftmat \Bmat^{(j-i)} \rightmat_{i,j \in \Iset}$.
...
% Let $\{a_h\}_{h \in \Iset}$ be the sequence of coefficients of the Toeplitz matrix $\Amat \in \Rbb^{n\times n}$ and let $\{\Bmat_h\}_{h \in \Iset}$ be the sequence of $m\times m$ blocks of the block Toeplitz matrix $\Bmat$.
% The complex-valued function $f(\omega) = \sum_{h \in \Iset} a_h e^{\ci h \omega}$ and the matrix-valued function $F(\omega) = \sum_{h \in \Iset} \Bmat_h e^{\ci h \omega}$ are the \emph{inverse Fourier transforms} of the sequences $\{a_h\}_{h \in \Iset}$ and $\{\Bmat_h\}_{h \in \Iset}$, with $\omega \in \Rbb$.
% From these two functions, one can recover these two sequences using the standard Fourier transform:
% \begin{equation}
%     a_h = \frac{1}{2\pi} \int_0^{2\pi} e^{-\ci h \omega} f(\omega) d\omega \quad \quad \Bmat_h = \frac{1}{2\pi} \int_0^{2\pi} e^{-\ci h \omega} F(\omega) d\omega .
...
% \end{equation}
%
% Note that if $f$ is the inverse Fourier transform of $\{a_h\}_{h \in \Iset}$, then $\Tmat(f)$ is equal to $\Amat$.
% Also, if $F$ is the inverse Fourier transform of $\{\Bmat_h\}_{h \in \Iset}$ as defined above, then the integral in \Cref{equation:expression_toeplitz_matrix} is matrix-valued, and thus $\Tmat(F) \in \Rbb^{mn \times mn}$ is the block matrix $\Bmat$.
%
...
Let $\Hmat^{\alpha_p} (f) = \leftmat \Hmat^{\alpha_p}_{i,j}(f) \rightmat_{i,j \in \Iset_0}$ where $\Hmat^{\alpha_p}_{i,j}$ is defined as:
\begin{equation}
