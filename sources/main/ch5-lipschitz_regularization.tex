...
% For a 2-dimensional signal $\mathbf{x} \in \Rbb^{n \times n}$ and a kernel $\mathbf{k} \in \Rbb^{m \times m}$ with $m$ odd, the convolution operation can be written as follows:
...
% The $\reshape: \Rbb^{n \times n} \rightarrow \Rbb^{n^2}$ operator is defined as follows: $\reshape(\mathbf{x})_q = \mathbf{x}_{\lfloor q/n \rfloor,\ q\mod n}$.
% The $\pad: \Rbb^{n \times n} \rightarrow \Rbb^{(n+2p) \times (n+2p)}$ operator is a zero-padding operation which takes a signal $\mathbf{x}$ of shape $\Rbb^{n \times n}$ and adds $0$ on the edges so as to obtain a new signal $\mathbf{y}$ of shape $\Rbb^{(n+2p) \times (n+2p)}$.
...
% Let us define a kernel $\mathbf{k} \in \Rbb^{3\times3}$ as follows:
% \begin{equation}
...
\end{equation}
The function $f_{\Amat}$ is said to be the \emph{generating function} of $\Amat$. To recover the Toeplitz matrix from its generating function, we have the following operator presented in \Cref{subsection:ch5-bound_on_the_singular_value_of_toeplitz_and_block_toeplitz_matrices} of the main paper:
...
The function $f_{\Bmat}$ is said to be the \emph{generating function} of $\Bmat$. To recover the block Toeplitz matrix from its generating function, we use the Toeplitz operator defined in \Cref{equation:toeplitz_operator}. We can show that $\Tmat(f_\Bmat) = \Bmat$:
...
Let $\{a_\seqidx\}_{\seqidx \in \seqsetN}$ be the sequence of coefficients of the Toeplitz matrix $\Amat \in \Rbb^{n\times n}$ and let $\{\Bmat_\seqidx\}_{\seqidx \in \seqsetN}$ be the sequence of $m\times m$ blocks of the block Toeplitz matrix $\Bmat$.
...
Also, if $F$ is the inverse Fourier transform of $\{\Bmat_\seqidx\}_{\seqidx \in \seqsetN}$ as defined above, then the integral in \Cref{equation:expression_toeplitz_matrix} is matrix-valued, and thus $\Tmat(F) \in \Rbb^{mn \times mn}$ is the block matrix $\Bmat$.
...
We are now able to combine \Cref{theorem:teoplitz_sup_singular,theorem:block_teoplitz_sup_singular} to bound the maximal singular value of doubly-block Toeplitz matrices with respect to their generating functions. 
...
\begin{maintheorem}[Bound on the maximal singular value of a Doubly-Block Toeplitz Matrix] \label{theorem:doubly_block_teoplitz_sup_singular}
...
Before proving \Cref{theorem:bound_sv_stacked_dbt}, we generalize the famous Widom identity \cite{widom1976asymptotic} that express the relation between Toeplitz and Hankel matrix to doubly-block Toeplitz and Hankel matrices.
...
\begin{lemma}[Generalization of Widom Identity]
...
Now we can prove \Cref{theorem:bound_sv_stacked_dbt} which bounds the maximal singular value of vertically stacked doubly-block Toeplitz matrices with their generating functions. 
...
\begin{maintheorem}[Bound on the maximal singular value on the convolution operation] \label{theorem:bound_max_sv_convolution} 
...
First, in order to prove \Cref{theorem:bound_max_sv_convolution}, we will need the following lemma which bound the singular values of a matrix constructed from the concatenation of multiple matrix. 
...
\end{equation}
\end{lemma}
\begin{proof}[\Cref{theorem:bound_concatenation_matrices}]
\begin{align}
...
\begin{proof}[\Cref{theorem:bound_max_sv_convolution}]
...
We can easily express the bound in \Cref{theorem:bound_max_sv_convolution} with the values of a 4-dimensional kernel.
...
In the rest of the paper, we will refer to the bound in \Cref{theorem:bound_max_sv_convolution} applied to a kernel as $\lipbound$ and we denote $\lipbound(\mathbf{k})$ the Lipschitz upper bound of the convolution performed by the kernel $\mathbf{k}$. 
...
This section aims at analyzing the bound introduced in \Cref{theorem:bound_max_sv_convolution}.
...
In order to compute $\lipbound$ from \Cref{theorem:bound_max_sv_convolution}, we have to compute the maximum modulus of several trigonometric polynomials.
...
We found that for 2-dimensional kernels, a simple grid search algorithm such as PolyGrid (see \Cref{algorithm:PolyGrid}), works better than more sophisticated approximation algorithms (\eg ~\citet{green1999calculating,de2009finding}).
...
\todotext{preciser farnia power method with 10 iter}
...
\todo{inclure les citations}
\cite{krizhevsky2012imagenet}, \cite{he2016deep}, \cite{huang2017densely}, \cite{simonyan2014very}, \cite{zagoruyko2016wide}, \cite{iandola2016squeezenet}
\todo{add comment on table}
