...
To build robust neural networks, \cite{cisse2017parseval} and ~\cite{NIPS2019_9673} have proposed to constrain the Lipschitz constant of neural networks by using orthogonal convolutions.
...
\begin{theorem}[Bound on the singular values of Toeplitz matrices] \label{th:teoplitz_sup_singular}
...
\begin{theorem}[Bound on the singular values of Block Toeplitz matrices ~\cite{gutierrez2012block}] \label{th:block_teoplitz_sup_singular}
...
\begin{theorem}[Bound on the maximal singular value of a Doubly-Block Toeplitz Matrix] \label{th:doubly_block_teoplitz_sup_singular}
...
\section{Bound on the Singular Values of Convolutional Layers}\label{sec:bound_lipschitz_cst_conv}
...
\begin{theorem}[Bound on the maximal singular value of stacked Doubly-block Toeplitz matrices] \label{th:theorem4} 
...
In order to prove Theorem~\ref{th:theorem4}, we have generalized the famous Widom identity~\cite{widom1976asymptotic}  expressing the relation between Toeplitz and Hankel matrices to doubly-block Toeplitz matrices. 
...
\begin{theorem}[\textbf{Main Result}: Bound on the maximal singular value on the convolution operation] \label{th:theorem5} 
...
%     \centering
%     \includegraphics[scale=0.36]{figures/chapter4/convergence_bound.pdf}
%     \caption{This graph represents the function $\Gamma(n)$ defined in Section~\ref{sec:analysis_tightness_bound} for different kernel size.}
%     \label{fig:convergence_bound}
%   \end{minipage}
%   \hfill
%   \begin{minipage}{.48\linewidth}
%     \centering
...
%     \label{algo:PolyGrid}
%     \begin{algorithmic}
%         \INPUT polynomial $f$, number of samples $S$
%         \OUTPUT approximated maximum modulus of $f$
%         \STATE $\sigma \gets 0$, $\omega_1 \gets 0$, $\epsilon \leftarrow 2\pi / S$
%         \FOR{$i=0$ {\bfseries to} $S-1$}
%             \STATE $\omega_1 \gets \omega_1 + \epsilon$, $\omega_2 \gets 0$
%             \FOR{$j=0$ {\bfseries to} $S-1$}
%                 \STATE $\omega_2 \gets \omega_2 + \epsilon$ 
%                 \STATE $\sigma \gets \max( \sigma, f(\omega_1, \omega_2))$
%             \ENDFOR
%         \ENDFOR
%         \STATE {\bfseries return} $\sigma$
%     \end{algorithmic}
%     \end{algorithm}
%   \end{minipage}
% \end{figure*}%
...
To compute the exact largest singular value of $\mathbf{M}^{(n)}$ for a specific $n$, we use the Implicitly Restarted Arnoldi Method (IRAM) ~\cite{lehoucq1996deflation} available in SciPy.
...
\begin{equation}
    ...
    \end{pmatrix}
\end{equation}
% A convolution operation can be written as a matrix multiplication as follows:
% \begin{equation}
%     \reshape(\mathbf{y}) = \reshape(\pad(\mathbf{x}) * \mathbf{k}) \Leftrightarrow \mathbf{y} = \mathbf{M} \reshape(\mathbf{x})
% \end{equation}
% where $\mathbf{x}, \mathbf{y} \in \mathbf{R}^{n \times n}$. 
...
The trigonometric polynomial that {\em generates} the Toeplitz matrix $\mathbf{A}$ can be defined as follows:
\begin{equation}
...
The same reasoning can be applied to block Toeplitz matrices. Instead of being complex-valued, the trigonometric polynomial that {\em generates} the block Toeplitz $\mathbf{B}$ is matrix-valued and can be defined as follows:
\begin{equation}
...
% \begin{customtheorem}{\ref{th:doubly_block_teoplitz_sup_singular}}[Bound on the Maximal Singular Value of a Doubly-Block Toeplitz Matrix]
% Let $\mathbf{D}(f) \in \mathbb{R}^{nm \times nm}$ be a doubly-block Toeplitz matrix generated by the function $f$, then:
% \begin{align}
% \sigma_{1} \left( \mathbf{D}(f) \right) &\leq \sup_{\omega_1, \omega_2 \in [0, 2\pi]^2}|f(\omega_1,\omega_2)|
% \end{align}
...
% \end{equation}
% where $d_{h_{1},h_{2}}$
% is the ${h_2}^\textrm{th}$ scalar of the ${h_1}^\textrm{th}$ block of the doubly-Toeplitz matrix $\mathbf{D}(f)$.
% \end{customtheorem}
...
% Recall that $\leftmat \mathbf{T}(F)\rightmat_{i,j} = \mathbf{T}_{i-j}$. 
...
% and let $\leftmat F(\omega_1) \rightmat_{i, j} = t_{i-j}$. 
...
% where $\gamma(h_2) = \max(0, -h_2)$, $\delta(h_2) = \max(0, h_2)$ and $\psi(h_1, h2) = \max(0, -n h_1) + \max(0, -h_2)$ and $\phi(h_1, h_2) = \max(0, n h_1) + \max(0, h_2)$. More explicitly, the $\gamma$ and $\beta$ function convert the Toeplitz index $h_2$ into a matrix indexing $(i, j$ and the functions $\phi$ and $\psi$ convert the doubly-block indexing (\ie $h_1$ is the id of the block and $h_2^{th}$ of the value in the block) into a matrix indexing $(i, j)$. And because the function $f(\omega_1,\ \cdot\ )$ is the generating function of $F(\omega_1)$ is it easy to show that the function $f$ is the generating function of $\mathbf{T}(F)$. Therefore, $\mathbf{T}(F) = \mathbf{D}(f)$ which concludes the proof. 
\end{proof}
...
In order to prove Theorem~\ref{th:theorem4}, we will need the following lemmas:

\begin{lemma}[\cite{zhang2011matrix}] \label{appendix-th:diff_positive_semidefinite_matrices}
...
\begin{lemma}[\cite{serra1994preconditioning}] \label{appendix-th:block_toeplitz_positive_definite}
...
\begin{lemma}[\cite{serra1994preconditioning}] \label{appendix-th:block_toeplitz_hermitian}
...
\begin{lemma}[\cite{gutierrez2012block}] \label{appendix-th:properties_block_toeplitz}
...
Let $\mathbf{H}^{\alpha_p} (f) = \leftmat \mathbf{H}^{\alpha_p}_{i,j}(f) \rightmat_{i,j \in \{0 \ldots n-1 \}}$ where $\mathbf{H}^{\alpha_p}_{i,j}$ is defined as:
...
% Let us simplify the notation and define the following operators:
% \begin{align*}
%     &\mathbf{H}_1(f) \triangleq \mathbf{H}^{\alpha_1}(f) \quad\quad\quad\quad \mathbf{H}_2(f) \triangleq \mathbf{H}^{\alpha_2}(f) \\
%     &\mathbf{H}_3(f) \triangleq \mathbf{H}^{\alpha_3}(f) \quad\quad\quad\quad \mathbf{H}_4(f) \triangleq \mathbf{H}^{\alpha_4}(f)
% \end{align*}
...
% Let $(i, j)$ be matrix indexes, let us define the following operators:
% \begin{align*}
%     i_1(i, n) &= \left\lfloor i/n \right\rfloor \quad \quad &&j_1(j, n) = \left\lfloor j/n \right\rfloor \\
%     i_2(i, n) &= i \mod n \quad \quad &&j_2(j, n) = j \mod n
% \end{align*}
% Functions $i_1$ and $j_1$ get us the block indexes based on the matrix indexes $i$ and $j$. $i_2$ and $j_2$ get us the coordinates of the value inside the block based on the matrix coordinates $(i, j)$. 
...
\end{align}
% In the Equation~\ref{appendix-eq:expression_fourier}, we have $h_1 = \left\lfloor j/n \right\rfloor - \left\lfloor i/n \right\rfloor$ and $h_2 = (j \mod n) - (i \mod n)$.
...
% Let us define $\hat{f}_{\alpha_k(i, j), \beta_l(i, j)}$ as the 2 dimensional Fourier transform of the function $f$. $\hat{f}_{\alpha_k(i, j), \beta_l(i, j)}$ refer to the $(\alpha_k(i, j), \beta_l(i, j))$ Fourier coefficient of $f$. We have $\leftmat \mathbf{H}_{\alpha_k, \beta_l}(f) \rightmat_{i,j} = \hat{f}_{\alpha_k(i, j), \beta_l(i, j)}$.  
...
% \begin{align}
%     (\mathbf{D}(fg))_{i, j} &= \frac{1}{2\pi} \int_{[0, 2\pi]^2} e^{-\ci (\omega_1 (j_1 - i_1) + \omega_2 (j_2 - i_2))} f(\omega_1, \omega_2) g(\omega_1, \omega_2) \,d(\omega_1, \omega_2) \\
%     &= \frac{1}{2\pi} \int_{[0, 2\pi]^2} e^{-\ci (\omega_1 (j_1 - i_1) + \omega_2 (j_2 - i_2))} \left( \sum_{h_1 \in N} \sum_{h_2 \in N} a_{h_1, h_2} e^{(h_1 \omega_1 + h_2 \omega_2)} \right) \left( \sum_{h_1 \in N} \sum_{h_2 \in N} b_{h_1', h_2'} e^{(h_1' \omega_1 + h_2' \omega_2)} \right) \,d(\omega_1, \omega_2) \\
%     &= \frac{1}{2\pi} \int_{[0, 2\pi]^2} e^{-\ci (\omega_1 (j_1 - i_1) + \omega_2 (j_2 - i_2))} \left( \sum_{h_1, h_2 \in X} \sum_{h_1', h_2' \in X} a_{h_1, h_2} e^{\ci (h_1 \omega_1 + h_2 \omega_2)} b_{h_1', h_2'} e^{\ci (h_1' \omega_1 + h_2' \omega_2)} \right) \,d(\omega_1, \omega_2) \\
%     &= \frac{1}{2\pi} \int_{[0, 2\pi]^2} e^{-\ci (\omega_1 (j_1 - i_1) + \omega_2 (j_2 - i_2))} \left( \sum_{h_1, h_2 \in X} \sum_{h_1', h_2' \in X} a_{h_1, h_2} b_{h_1', h_2'} e^{\ci ((h_1 + h_1') \omega_1 + (h_2 + h_2') \omega_2)} \right) \,d(\omega_1, \omega_2) \\
%     &= \frac{1}{2\pi} \int_{[0, 2\pi]^2} \left( \sum_{h_1, h_2 \in X} \sum_{h_1', h_2' \in X} a_{h_1, h_2} b_{h_1', h_2'} e^{\ci ((h_1 + h_1' + j_1 - i_1) \omega_1 + (h_2 + h_2' + j_2 - i_2) \omega_2)} \right) \,d(\omega_1, \omega_2) \\
%     &= \sum_{h_1, h_2 \in X} \sum_{h_1', h_2' \in X} a_{h_1, h_2} b_{h_1', h_2'} \frac{1}{2\pi} \int_{[0, 2\pi]^2} e^{\ci ((h_1 + h_1' + j_1 - i_1) \omega_1 + (h_2 + h_2' + j_2 - i_2) \omega_2)} \,d(\omega_1, \omega_2) \\
% \end{align}
...
\end{align}
% where $P = \{ (k_1, k_2)\ |\ k_1, k_2 \in \{0, \dots, n-1 \} \}$
where $P = \{ (k_1, k_2)\ |\ k_1, k_2 \in \mathbb{N} \cup 0, 0 \leq k_1 \leq n-1,  0 \leq k_2 \leq n-1 \}$.


% From there, we can simplify the notation greatly by defining new doubly-block Hankel operators:
% \begin{align}
%     &\mathbf{H}_1(f) = \mathbf{H}^{(n)}_{\alpha_1, \beta_0}(f) \quad\quad\quad\quad \mathbf{H}_2(f) = \mathbf{H}^{(n)}_{\alpha_0, \beta_1}(f) \\
%     &\mathbf{H}_3(f) = \mathbf{H}^{(n)}_{\alpha_1, \beta_1}(f) \quad\quad\quad\quad \mathbf{H}_4(f) = \mathbf{H}^{(n)}_{\alpha_2, \beta_1}(f)
% \end{align}
% where:
% \begin{equation}
%     \leftmat \mathbf{H}^{(n)}_{\alpha, \beta}(f) \rightmat_{i,j} = \frac{1}{4\pi^2} \int_{[0, 2\pi]^2} e^{-\mathbf{i} \leftmat \alpha(i, j) \omega_1 + \beta(i, j) \omega_2 \rightmat} f(\omega_1, \omega_2) \,d(\omega_1, \omega_2)
% \end{equation}
% with 
% \begin{itemize}
%     \item[] $\alpha_0(i, j) = \lfloor j/n \rfloor - \lfloor i / n \rfloor $
%     \item[] $\alpha_1(i, j) = \lfloor j/n \rfloor + \lfloor i / n \rfloor + 1$
%     \item[] $\alpha_2(i, j) = \lfloor j/n \rfloor - \lfloor i / n \rfloor - n$
%     \item[] $\beta_0(i, j) = (j \mod n) - (i \mod n )$
%     \item[] $\beta_1(i, j) = (j \mod n) + (i \mod n ) + 1$
% \end{itemize}
...
\begin{flalign*}
    % # H1_a_.T @ H1_b
    \leftmat \mathbf{H}^{\alpha_1 \top}(f^*) \mathbf{H}^{\alpha_1}(g) \rightmat_{i, j} &=  \sum_{k_1, k_2 \in P} \hat{f}^*_{(k_1+i_1+1),(i_2-k_2)} \hat{g}_{(j_1+k_1+1),(j_2-k_2)} \\
    &=  \sum_{k_1, k_2 \in P} \hat{f}_{(-k_1-i_1-1),(k_2-i_2)} \hat{g}_{(j_1+k_1+1),(j_2-k_2)} \\[10pt]
    % # H2_a_.T @ H2_b
    \leftmat \mathbf{H}^{\alpha_2 \top}(f^*) \mathbf{H}^{\alpha_2}(g) \rightmat_{i, j} &=  \sum_{k_1, k_2 \in P} \hat{f}^*_{(i_1-k_1),(k_2+i_2+1)} \hat{g}_{(j_1-k_1),(j_2+k_2+1)} \\
    &=  \sum_{k_1, k_2 \in P} \hat{f}_{(k_1-i_1),(-k_2-i_2-1)} \hat{g}_{(j_1-k_1),(j_2+k_2+1)} \\[10pt]
    % # H3_a_.T @ H3_b
    \leftmat \mathbf{H}^{\alpha_3 \top}(f^*) \mathbf{H}^{\alpha_3}(g) \rightmat_{i, j} &=  \sum_{k_1, k_2 \in P} \hat{f}^*_{(k_1+i_1+1),(k_2+i_2+1)} \hat{g}_{(j_1+k_1+1),(k_2+j_2+1)} \\
    &= \sum_{k_1, k_2 \in P} \hat{f}_{(-k_1-i_1-1),(-k_2-i_2-1)} \hat{g}_{(j_1+k_1+1),(k_2+j_2+1)} \\[10pt]
    % # H4_a_.T @ H4_b
    \leftmat \mathbf{H}^{\alpha_4 \top}(f^*) \mathbf{H}^{\alpha_4}(g) \rightmat_{i, j} &= \sum_{k_1, k_2 \in P} \hat{f}^*_{(i_1-k_1-n),(k_2+i_2+1)} \hat{g}_{(j_1-k_1-n),(j_2+k_2+1)} \\
...
\begin{flalign*}
    ...
    \Leftrightarrow \leftmat \mathbf{Q} \mathbf{H}^{\alpha_1 \top}(f) \mathbf{H}^{\alpha_1}(g^*) \mathbf{Q} \rightmat_{i, j} &= \sum_{k_1, k_2 \in P} \hat{f}_{(k_1-i_1+n),(k_2-i_2)} \hat{g}_{(j_1-k_1-n),(j_2-k_2)} \\[10pt]
    % # Y @ H2_a.T @ H2_b_ @ Y
    \leftmat \mathbf{H}^{\alpha_2 \top}(f) \mathbf{H}^{\alpha_2}(g^*) \rightmat_{i, j} &=  \sum_{k_1, k_2 \in P} \hat{f}_{(i_1-k_1),(k_2+i_2+1)} \hat{g}^*_{(j_1-k_1),(j_2+k_2+1)} \\
    &=  \sum_{k_1, k_2 \in P} \hat{f}_{(i_1-k_1),(k_2+i_2+1)} \hat{g}_{(k_1-j_1),(-j_2-k_2-1)} \\
    \Leftrightarrow \leftmat \mathbf{Q} \mathbf{H}^{\alpha_2 \top}(f) \mathbf{H}^{\alpha_2}(g^*) \mathbf{Q} \rightmat_{i, j} &=  \sum_{k_1, k_2 \in P} \hat{f}_{(k_1-i_1),(k_2-i_2+n)} \hat{g}_{(j_1-k_1),(j_2-k_2-n)} \\[10pt]
    % # Y @ H3_a.T @ H3_b_ @ Y
    \leftmat \mathbf{H}^{\alpha_3 \top}(f) \mathbf{H}^{\alpha_3}(g^*) \rightmat_{i, j} &=  \sum_{k_1, k_2 \in P}  \hat{f}_{(k_1+i_1+1),(k_2+i_2+1)} \hat{g}^*_{(j_1+k_1+1),(k_2+j_2+1)} \\
    &=  \sum_{k_1, k_2 \in P} \hat{f}_{(k_1+i_1+1),(k_2+i_2+1)} \hat{g}_{(-j_1-k_1-1),(-k_2-j_2-1)} \\
    \Leftrightarrow \leftmat \mathbf{Q} \mathbf{H}^{\alpha_3 \top}(f) \mathbf{H}^{\alpha_3}(g^*) \mathbf{Q} \rightmat_{i, j} &=  \sum_{k_1, k_2 \in P} \hat{f}_{(k_1-i_1+n),(k_2-i_2+n)} \hat{g}_{(j_1-k_1-n),(-k_2+j_2-n)} \\[10pt]
    % # Y @ H4_a.T @ H4_b_ @ Y
    \leftmat \mathbf{H}^{\alpha_4 \top}(f) \mathbf{H}^{\alpha_4}(g^*) \rightmat_{i, j} &=  \sum_{k_1, k_2 \in P}  \hat{f}_{(-k_1+i_1-n),(k_2+i_2+1)} \hat{g}^*_{(j_1-k_1-n),(j_2+k_2+1)} \\
    &= \sum_{k_1, k_2 \in P} \hat{f}_{(-k_1+i_1-n),(k_2+i_2+1)} \hat{g}_{(-j_1+k_1+n),(-j_2-k_2-1)} \\
    \Leftrightarrow \leftmat \mathbf{Q} \mathbf{H}^{\alpha_4 \top}(f) \mathbf{H}^{\alpha_4}(g^*) \mathbf{Q} \rightmat_{i, j} &= \sum_{k_1, k_2 \in P} \hat{f}_{(-k_1-i_1-1),(k_2-i_2+n)} \hat{g}_{(j_1+k_1+1),(j_2-k_2-n)}
\end{flalign*}
}
...
Now we can state our theorem which bounds the maximal singular value of vertically stacked doubly-block Toeplitz matrices with their generating functions. 

% \begin{customtheorem}{\ref{th:theorem4}}[Bound on the maximal singular value of stacked Doubly-block Toeplitz matrices]
% Consider doubly-block Toeplitz matrices $\mathbf{D}(f_1), \dots, \mathbf{D}(f_{\cin})$ where $f_i: \mathbb{R}^2 \rightarrow \mathbb{C}$ is a generating function. Construct a matrix $\mathbf{M}$ with $\cin\times n^2$ rows and $n^2$ columns, as follows:
% \begin{equation}
%     \mathbf{M} \triangleq \leftmat \mathbf{D}^\top(f_1), \dots, \mathbf{D}^\top(f_{\cin}) \rightmat^\top .
% \end{equation}
% Then, we can bound the maximal singular value of the matrix $\mathbf{M}$ as follows:
% \begin{align}
%     \sigma_1\left(\mathbf{M} \right) &\leq \sup_{\omega_1, \omega_2 \in \left[0, 2\pi\right]^2} \sqrt{ \sum_{i=1}^{\cin} \left|f_i\right (\omega_1, \omega_2)|^2} .
% \end{align}
% \end{customtheorem}
...
% \begin{customtheorem}{\ref{th:theorem4}}
% Let us consider a matrix of vertically stacked doubly-block Toeplitz matrices. Let $\mathbf{M} \in \mathbb{R}^{kn^2 \times n^2}$ defined as follows:
% \begin{equation}
%     \mathbf{M} \triangleq \leftmat\mathbf{D}^{\top}(f_1), \dots,  \mathbf{D}^{\top}(f_k) \rightmat^\top
% \end{equation}
% where $f_i: \mathbb{R}^2 \rightarrow \mathbb{C}$. 

% Then, we can bound the maximal singular value of the matrix $\mathbf{M}$ as follows:
% \begin{equation}
%     \sigma_1 \left( \mathbf{M} \right) \leq \sup_{\omega_1, \omega_2 \in [0, 2\pi]^2} \sqrt{\sum_{i=1}^k \left| f_i(\omega_1, \omega_2) \right|^2} \label{appendix-eq:bound}
% \end{equation}
% \end{customtheorem}
...
\end{align}
% Note that we can use Theorem \ref{th:doubly_block_teoplitz_sup_singular} because the matrix $ \mathbf{D} \left( \sum_{i=1}^{\cin} |f_i|^2 \right)$ is generated by a positive function therefore it is symmetric and the eigenvalue is equal to the singular value. 
...
% In the context of Toeplitz and block Toeplitz matrix, this matrix refer to the famous Widom identity \cite{widom1976asymptotic}. Let us define the Toeplitz and Hankel operators as follows:
% \begin{equation}
% \mathbf{T}(\phi) = \left( \hat{\phi}_{i-j} \right)_{i, j \geq 0} \quad \mathbf{H}(\phi) = \left( \hat{\phi}_{i+j+1} \right)_{i, j \geq 0}
% \end{equation}
% where $(\hat{\phi}_n)$ stands for the sequence of Fourier coefficients of $\phi$.
% The identity between Toeplitz and Hankel operator is as follows:
% \begin{equation}
% \mathbf{T}(\phi \psi) - \mathbf{T}(\phi) \mathbf{T}(\psi) = \mathbf{H}(\phi) \mathbf{H}(\psi^*)
% \end{equation}

% The relation in the finite case is define as follows:
% \begin{equation}
% \mathbf{T}_n(\phi \psi) - \mathbf{T}_n(\phi) \mathbf{T}_n(\psi) = \mathbf{P}_n \mathbf{H}(\phi) \mathbf{H}(\psi^*) \mathbf{P}_n + \mathbf{Q} \mathbf{H}(\phi^*) \mathbf{H}(\psi) \mathbf{Q}
% \end{equation}
% where:
% \begin{align}
% \mathbf{P}_n(x_0,x_1,x_2,\dots) &= (x_0,x_1,\dots,x_n,0,\dots) \\
% \mathbf{Q}(x_0,x_1,x_2,\dots) &= (x_n,x_{n-1},\dots,x_0,0,\dots)
% \end{align}

We know from Lemma~\ref{appendix-th:widom_idenity} that 
%the matrix
%\begin{equation}
%    \mathbf{D} \left( f g \right) - \mathbf{D} \left(f \right) \mathbf{D} (g)
%\end{equation}
...
% From equation \ref{appendix-eq:widom_identity_block_topelitz}, we can see that the matrix \begin{equation}
%     \mathbf{D}(|f|^2) - \mathbf{D}^{\top}(f)\mathbf{D}(f)
% \end{equation}
% is positive semi-definite because it can be decompose into a sums of positive semi-definite matrices. 
...
\subsection{Proof of Theorem~\ref{th:theorem5} -- Bound on the Maximal Singular Value on the Convolution Operation}
...
% \begin{customtheorem}{\ref{th:theorem5}}[\textbf{Main Result}: Bound on the maximal singular value on the convolution operation]
% Let us define doubly-block Toeplitz matrices $\mathbf{D}(f_{11}), \dots, \mathbf{D}(f_{\cin\times \cout})$ where $f_{ij}: \mathbb{R}^2 \rightarrow \mathbb{C}$ is a generating function. Construct a matrix $\mathbf{M}$ with $\cin\times n^2$ rows and $\cout\times n^2$ columns such as
% \begin{equation}
%     \mathbf{M} \triangleq  \leftmat\begin{array}{ccc}
%     \mathbf{D}(f_{11}) & \cdots & \mathbf{D}(f_{1,\cout})   \\
%     \vdots & & \vdots   \\
%     \mathbf{D}(f_{\cin,1}) & \cdots & \mathbf{D}(f_{\cin,\cout}) \\
%     \end{array}\rightmat .
% \end{equation}
% Then, with $f_{ij}$ a multivariate polynomial of the same form as Equation~\ref{eq:muli_variate_poly_on_M}, we have:
% \begin{equation}
%    \sigma_1(\mathbf{M}) \leq \sqrt{ \sum_{i=1}^{\cout} \sup_{\omega_1, \omega_2 \in [0, 2\pi]^2} \sum_{j = 1}^{\cin} \left|f_{ij}(\omega_1, \omega_2) \right|^2 } .
% \end{equation}
% \end{customtheorem}

% \begin{customtheorem}{5}[\textbf{Main Result}] ~\\ 
% Let us define doubly block Toeplitz matrices $\mathbf{D}(f_{11}), \dots, \mathbf{D}(f_{pq})$ where $f_{ij}: \mathbb{R}^2 \rightarrow \mathbb{C}$ is a generating function. Construct a matrix $\mathbf{M}$ of size $pn^2 \times qn^2$ such as:
% {\small
% \begin{equation}
%     \mathbf{M} = \leftmat\begin{array}{ccc}
%     \mathbf{D}(f_{11}) & \cdots & \mathbf{D}(f_{1q})   \\
%     \vdots & & \vdots   \\
%     \mathbf{D}(f_{p1}) & \cdots & \mathbf{D}(f_{pq}) \\
%     \end{array}\rightmat
% \end{equation}}
% Then:
% \begin{equation}
%   \sigma_1(\mathbf{M}) \leq \sqrt{ \sum_{j=0}^{q} \sup_{\omega_1, \omega_2 \in [0, 2\pi]^2} \sum_{i = 0}^{p} \left|f_{ij}(\omega_1, \omega_2) \right|^2 } 
% \end{equation}
% where the function $f_{ij}: \mathbb{R}^2 \rightarrow \mathbb{C}$, is a multivariate trigonometric polynomial of the form:
% \begin{equation}
%     f_{ij}(\omega_1, \omega_2) \triangleq \sum_{h_1 = -d}^{d} \sum_{h_2 = -d}^{d} d_{i, j, h_1, h_2} e^{\ci (h_1 \omega_1 + h_2 \omega_2)},
% \end{equation}
% where $d_{i, j, h_1, h_2}=\leftmat \mathbf{D}(f_{ij}) \rightmat_{a,b}$, with $a = \max(0, -n h_1) + \max(0, -h_2)$ and $b = \max(0, n h_1) + \max(0, h_2)$.
% \end{customtheorem}
...
\subsection{Additional Results and Discussion of Section~\ref{subsec:comparaison_sota}}
...
\begin{itemize}
    \item \makebox[3.2cm]{\cite{sedghi2018the}\hfill} \url{https://github.com/brain-research/conv-sv}
...
% \subsection{Additional Results of Section~\ref{sec:experiments} \& Hyper-parameters}
