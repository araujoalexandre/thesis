...
%
% In addition, the matrix-vector product is simplified from $\mathcal{O}(n^2)$ to $\mathcal{O}(n \log n)$ using the convolution theorem.
...
% Because this operation can be simplified to a simple element wise vector multiplication, the matrix multiplication $Cx$ can be computed in $\mathcal{O}(n \log n)$ instead of $\mathcal{O}(n^2)$.
